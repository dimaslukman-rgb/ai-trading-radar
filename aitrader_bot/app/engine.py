...
from .news_filter import get_upcoming_event, get_upcoming_events
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                dd.update(
                    ...
                    sentiment=senti,
                    volatility=vol,
                    news_events=get_upcoming_events(limit=5),
                    analysis=analysis,
                )
                ...
                if signal.action == "buy" and not has_pos:
                    # Calculate dynamic position size (gunakan equity real dari account)
                    try:
                    ...
                    if strategy.config.aggressive_mode and qty < 0.05:
                        # Basket-case: 1 posisi max, pakai lot maksimal
                        # max_trade_pct sebagai batas atas
                    ...
                    # ── Only log if order was actually filled ────────────────
                    if result.status == OrderStatus.FILLED:
                        ...
                        # ── Track entry time for timeout exit ──────────────────
                        ...
                        dd.add_trade("BUY", mapped_symbol, quote.last, qty, result.message, status="REJECTED")

                elif signal.action == "sell" and has_pos:
                    # ── OPPOSITE-SIGNAL EXIT ─────────────────────────────────
                    pos = positions[0]
                    pnl = quote.last - pos.avg_price if pos.avg_price else 0.0
                    pnl_pips = pnl * 10
                    result = self.broker.close_position(pos.ticket)
                    signal_msg = f"SOLD {mapped_symbol} @ {quote.last:.2f} P&L={pnl:+.2f} ({pnl_pips:+.0f}p) | OPPOSITE SIGNAL: {signal.reason}"
                    self._put(f"signal:{signal_msg}")
                    log.info(f"signal:{signal_msg}")
                    dd.add_trade("SELL (opp signal)", mapped_symbol, quote.last, pos.quantity, f"Opposite: {signal.reason}", pnl)
                    dd.add_log(f"SOLD {mapped_symbol} @ {quote.last:.2f} P&L={pnl_pips:+.0f}p (opposite signal)")
                    tg.send_signal("sell", mapped_symbol, quote.last, f"Opposite signal: {signal.reason}", pnl=pnl)
                    entry_time = None
                    dd.update(entry_time=None)  # Clear from dashboard

                else:
                    # ── Check SL / TP / TIMEOUT / lock profit for existing positions ──
                    forced_exit = None
                    if has_pos:
                        pos = positions[0]
                        now = datetime.now()
                    ...
                    if forced_exit:
                        result = self.broker.close_position(pos.ticket)
                        pnl = quote.last - pos.avg_price if pos.avg_price else 0.0
                        signal_msg = f"CLOSED {mapped_symbol} @ {quote.last:.2f} P&L={pnl:.2f} | {forced_exit}"
                        self._put(f"signal:{signal_msg}")
                        log.info(f"signal:{signal_msg}")
                        ...
                        dd.add_trade(trade_type, mapped_symbol, quote.last, pos.quantity, forced_exit, pnl)
                        dd.add_log(f"CLOSED {mapped_symbol} @ {quote.last:.2f} ({forced_exit})")
                        tg.send_signal("sell", mapped_symbol, quote.last, forced_exit, pnl=pnl)
                        entry_time = None  # Reset entry timer
                        dd.update(entry_time=None)  # Clear from dashboard
                    else:
                        hold_label = "HODL" if has_pos else "HOLD"
