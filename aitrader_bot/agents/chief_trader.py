...
class ChiefTrader(BaseAgent):
    ...
    def _decide(self, results: dict[str, AgentResult], ctx: AgentContext) -> ChiefDecision:
        ...
        if session_result and session_result.output.get("quality", 50) < 10:
        ...
        if total_score > 0 and confidence_pct >= 20:
            decision.action = "BUY"
            decision.confidence = round(confidence_pct, 1)
            decision.reason = f"Bullish bias ({bullish_score}-{bearish_score})"
            decision.approved = True
        elif total_score < 0 and confidence_pct >= 20:
            decision.action = "SELL"
            decision.confidence = round(confidence_pct, 1)
            decision.reason = f"Bearish bias ({bullish_score}-{bearish_score})"
            decision.approved = True
        else:
            decision.action = "HOLD"
            decision.confidence = round(100 - confidence_pct, 1)
            decision.reason = f"Neutral / conflicting signals ({bullish_score}-{bearish_score})"
